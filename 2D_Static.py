...
parser = argparse.ArgumentParser(description='Traditional 2D RNN for 2D J1J2')
...
if not os.path.exists(f'../Results/2D_J1J2/{args.Nx}x{args.Ny}/Traditional/{args.NAME}/'):
    os.makedirs(f'../Results/2D_J1J2/{args.Nx}x{args.Ny}/Traditional/{args.NAME}/Outputs/')
    os.makedirs(f'../Results/2D_J1J2/{args.Nx}x{args.Ny}/Traditional/{args.NAME}/Params/')

with open(__file__, 'r') as current_file:
    code_content = current_file.read()
    
with open(f'../Results/2D_J1J2/{args.Nx}x{args.Ny}/Traditional/{args.NAME}/train_backup.py', 'w') as backup_file:
...
while epoch <= 200000:
    if epoch != 1 and epoch % (interval) == 1:
        ...
        with open(f'../Results/2D_J1J2/{args.Nx}x{args.Ny}/Traditional/{args.NAME}/Params/{epoch-1}_params.pkl', 'wb') as f:
        ...
        results.to_csv(f'../Results/2D_J1J2/{args.Nx}x{args.Ny}/Traditional/{args.NAME}/Outputs/{epoch-1}_outputs.csv')
...
results = {'Energies': [float(x.real) for x in energies], 'Losses': [float(x) for x in losses],
            'Variations': [float(x) for x in variations], 'Time': durations}
df = pd.DataFrame(results)
with open(f'../Results/2D_J1J2/{args.Nx}x{args.Ny}/Traditional/{args.NAME}/Params/{epoch-1}_params.pkl', 'wb') as f:
    pickle.dump((params, opt_state), f)

save_path = f'../Results/2D_J1J2/{args.Nx}x{args.Ny}/Traditional/{args.NAME}/Outputs/{args.NAME}_outputs.csv'
df.to_csv(save_path)
