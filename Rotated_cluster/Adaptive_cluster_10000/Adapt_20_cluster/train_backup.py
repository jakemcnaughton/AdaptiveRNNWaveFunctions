...
def train(N, model_type):
    ...
    for dim in range(len(models)):
        ...
        if dim > 0:
            ...
            with open(f'./Adaptive_cluster_2000/{args.NAME}/Params/{dim}_{epoch}_params.pkl', 'wb') as f:
    ...
    with open(f'./Adaptive_cluster_2000/{args.NAME}/Params/{args.MAX_POWER}_{epoch}_params.pkl', 'wb') as f:
        pickle.dump(params, f)

    save_path = f'./Adaptive_cluster_2000/{args.NAME}/Outputs/{args.NAME}_{model_type}_{N}_outputs.csv'
    df.to_csv(save_path)
    print(f"Final Epoch = {epoch}")
    print(final_energy(params, key1, models[-1], args.N, 10000))

# Create Directories
if not os.path.exists(f'./Adaptive_cluster_2000/{args.NAME}/'):
    os.makedirs(f'./Adaptive_cluster_2000/{args.NAME}/Outputs/')
    os.makedirs(f'./Adaptive_cluster_2000/{args.NAME}/Params/')

# Save Code
with open(__file__, 'r') as current_file:
    code_content = current_file.read()
    
with open(f'./Adaptive_cluster_2000/{args.NAME}/train_backup.py', 'w') as backup_file:
