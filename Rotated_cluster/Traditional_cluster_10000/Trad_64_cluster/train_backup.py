...
def train(N, model_type):
    ...
    with open(f'./Traditional_cluster_10000/{args.NAME}/Params/{args.MAX_POWER}_{epoch}_params.pkl', 'wb') as f:
        pickle.dump(params, f)

    save_path = f'./Traditional_cluster_10000/{args.NAME}/Outputs/{args.NAME}_{model_type}_{N}_outputs.csv'
    df.to_csv(save_path)
    print(f"Final Epoch = {epoch}")
    print(final_energy(params, key1, model, N, 10000))


if not os.path.exists(f'./Traditional_cluster_10000/{args.NAME}/'):
    os.makedirs(f'./Traditional_cluster_10000/{args.NAME}/Outputs/')
    os.makedirs(f'./Traditional_cluster_10000/{args.NAME}/Params/')

with open(__file__, 'r') as current_file:
    code_content = current_file.read()
    
with open(f'./Traditional_cluster_10000/{args.NAME}/train_backup.py', 'w') as backup_file:
...
# Run traditional one for 50,000 with dim = 256 with LR = 1e-3
# Run adaptive starting at 2 going to 256 for total of 50,000 with LR = 1e-3
# 20, 40, 80, 100, 160, 200 Vanilla, GRU, LSTM