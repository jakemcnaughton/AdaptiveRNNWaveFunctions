...
from ClusterState_Helpers import get_loss, generate_models, param_transform_automatic, final_energy
from jax import jit
import pandas as pd
import time
import pickle
...
parser.add_argument('--NUMSTEPS', type=int, default=10000, help='')
...
def train(N, model_type):
    ...
    print(models[0].apply(params_init,x))
    ...
    for dim in range(len(models)):
        ...
        if dim > 0:
            ...
            with open(f'./Adaptive_cluster_{args.NUMSTEPS}/{args.NAME}/Params/{dim}_{epoch}_params.pkl', 'wb') as f:
    ...
    dict_0 = {'Energies': [complex(x) for x in energies], 'Losses': [float(x) for x in losses],
              'Variations': [float(x) for x in variations], 'Model dh': dimensions, 'Time': durations}
    df = pd.DataFrame(dict_0)
    with open(f'./Adaptive_cluster_{args.NUMSTEPS}/{args.NAME}/Params/{args.MAX_POWER}_{epoch}_params.pkl', 'wb') as f:
        pickle.dump(params, f)

    save_path = f'./Adaptive_cluster_{args.NUMSTEPS}/{args.NAME}/Outputs/{args.NAME}_{model_type}_{N}_outputs.csv'
    df.to_csv(save_path)

    finalE = final_energy(params, key1, models[-1], args.N, 10000)
    print(f"Final Epoch = {epoch}")
    print(finalE)

    save_path_data = f'./Adaptive_cluster_{args.NUMSTEPS}/{args.NAME}/Outputs/{args.NAME}_{model_type}_{N}_finaldata.csv'
    np.savetxt(save_path_data, finalE)

# Create Directories
if not os.path.exists(f'./Adaptive_cluster_{args.NUMSTEPS}/{args.NAME}/'):
    os.makedirs(f'./Adaptive_cluster_{args.NUMSTEPS}/{args.NAME}/Outputs/')
    os.makedirs(f'./Adaptive_cluster_{args.NUMSTEPS}/{args.NAME}/Params/')

# Save Code
with open(__file__, 'r') as current_file:
    code_content = current_file.read()
    
with open(f'./Adaptive_cluster_{args.NUMSTEPS}/{args.NAME}/train_backup.py', 'w') as backup_file:
